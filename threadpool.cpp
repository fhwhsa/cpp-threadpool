...
private:
    ...
    ThreadPool(size_t threads = std::thread::hardware_concurrency()) : stop(false) {
        ...
        adjustThread = std::thread([this] {
            ...
            while (!stop)
            {
                std::this_thread::sleep_for(std::chrono::milliseconds(ADJUST_DURATION));
                adjust_thread_count();
            }
        });
    }
    ...
    void adjust_thread_count() {
        ...
        // 增加工作线程
        if (tasks.size() * 2 > workers.size() && workers.size() < MAX_THREADS) {
            lock.unlock();  // 提前解锁，增加工作线程不影响现有工作线程
            int n = MAX_THREADS - workers.size();
            n = n > ADJUST_ADD_THREADS ? ADJUST_ADD_THREADS : n;
            while (n--)
            {
                workers.emplace_back([this] { worker_thread(); });
            }
        ...
        else if (workers.size() > MIN_THREADS) 
        {
            ...
            auto it = std::remove_if(workers.begin(), workers.end(), [&](std::thread& t) {
                ...
                if (static_cast<ulong>(dur.count()) > MAX_IDLE_TIME)
                {
                    t.detach();
    ...
    /// @brief 最长空闲时间（ms）
    static constexpr ulong MAX_IDLE_TIME = 1000 * 60 * 5;
    /// @brief 最大工作线程数
    static constexpr int MAX_THREADS = 16;
    /// @brief 最小工作线程数
    static constexpr int MIN_THREADS = 5;
    /// @brief 单次调整增加工作线程数的最大值
    static constexpr int ADJUST_ADD_THREADS = 5;
    /// @brief 执行调整函数的间隔时间(ms)
    static constexpr ulong ADJUST_DURATION = 1000 * 60 * 3;
};
